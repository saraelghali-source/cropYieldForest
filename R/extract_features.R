...
#' @param fun Fonctions statistiques zonales à calculer (défaut: mean, max, min, sd).
...
extract_features <- function(fields,
  ...
  if (!is.null(ndvi_stack)) {
    target_crs <- terra::crs(ndvi_stack)
  ...
  # ── Extraction NDVI ──────────────────────────────────────────────────────────
  if (!is.null(ndvi_stack)) {
    message("Extraction NDVI par parcelle (méthode: ", method, ") ...")
    ndvi_stats <- .extract_raster_stats(fields_proj, ndvi_stack, "ndvi", fun, method)
  ...
  if (!is.null(indices_stack)) {
    ...
    for (lyr in names(indices_stack)) {
      r     <- indices_stack[[lyr]]
      stats <- .extract_raster_stats(fields_proj, r, tolower(lyr), fun, method)
...
#'
#' Nettoie le dataset de features : supprime les doublons, impute les NA,
#' encode les variables catégorielles et sépare les données en jeux
#' d'entraînement et de test.
#'
#' @param features data.frame issu de \code{extract_features}.
#' @param target Nom de la colonne cible (défaut: "yield").
#' @param exclude_cols Colonnes à exclure du modèle (ex: identifiants).
...
#' @param impute_method Méthode d'imputation des NA numériques :
#'   "median" (défaut) ou "mean".
#' @param scale Centrer et réduire les variables numériques ? (défaut: FALSE).
#'
#' @return Liste avec :
#'   \describe{
#'     \item{train}{data.frame d'entraînement.}
#'     \item{test}{data.frame de test.}
#'     \item{features_used}{Noms des variables explicatives retenues.}
#'     \item{n_train}{Nombre d'observations d'entraînement.}
#'     \item{n_test}{Nombre d'observations de test.}
#'   }
#'
#' @examples
#' \dontrun{
...
preprocess_data <- function(features,
  ...
  # Imputation des NA numériques
  num_cols <- names(df)[sapply(df, is.numeric) & names(df) != target]
  for (col in num_cols) {
    na_count <- sum(is.na(df[[col]]))
    if (na_count > 0) {
      fill_val <- if (impute_method == "median") median(df[[col]], na.rm = TRUE) else
...
#' @keywords internal
.extract_raster_stats <- function(sf_obj, raster, prefix, fun, method) {

  if (method == "exact" && requireNamespace("exactextractr", quietly = TRUE)) {
    stats <- exactextractr::exact_extract(raster, sf_obj, fun, progress = FALSE)
    if (is.numeric(stats)) {
      stats <- as.data.frame(stats)
      names(stats) <- paste0(prefix, "_", fun[1])
    } else {
      names(stats) <- paste0(prefix, "_", names(stats))
    }
  } else {
    # Fallback centroïde
    pts   <- sf::st_centroid(sf_obj)
    vals  <- terra::extract(raster, terra::vect(pts))
    vals  <- vals[, -1, drop = FALSE]
    stats <- data.frame(
      mean = apply(vals, 1, mean, na.rm = TRUE),
      max  = apply(vals, 1, max,  na.rm = TRUE),
      min  = apply(vals, 1, min,  na.rm = TRUE),
      sd   = apply(vals, 1, sd,   na.rm = TRUE)
    )
    names(stats) <- paste0(prefix, "_", names(stats))
  }
