...
#'
#' Applique le modèle Random Forest entraîné à l'ensemble du raster
#' de la zone d'étude pour générer une carte continue de rendement prédit.
#' Exporte le résultat en GeoTIFF.
#'
...
#' @param weather_df data.frame météo (sera assigné uniformément à chaque pixel).
#' @param features_used Vecteur des noms de variables du modèle
#'   (model_result$features_used ou data_ml$features_used).
...
#' )
#' terra::plot(yield_map, main = "Rendement prédit (t/ha)")
#' }
#'
#' @export
predict_yield_map <- function(model_result,
  ...
  if (!is.null(features_used)) {
    available <- intersect(features_used, names(pixel_df))
    missing   <- setdiff(features_used, names(pixel_df))
    if (length(missing) > 0) {
      warning("Variables manquantes pour la prédiction (remplacement par 0) : ",
              paste(missing, collapse = ", "))
      for (m in missing) pixel_df[[m]] <- 0
    }
    pixel_df <- pixel_df[, features_used, drop = FALSE]
  }

  # Gestion des NA (pixels hors couverture)
  ...
  if (sum(!na_mask) > 0) {
    pred_vals[!na_mask] <- stats::predict(model_result$model,
...
#'
#' Génère des cartes du rendement prédit avec ggplot2 :
#' rendement absolu, zones à faible rendement et variabilité spatiale.
#' Les cartes sont exportées en PNG et/ou PDF.
#'
#' @param yield_rast SpatRaster issu de \code{predict_yield_map}.
#' @param fields Objet sf des parcelles (optionnel, superposé sur la carte).
#' @param low_threshold Seuil en-dessous duquel le rendement est "faible" (défaut: 2).
#' @param out_dir Répertoire de sortie (défaut: "outputs/").
...
#'
#' Génère un barplot horizontal de l'importance des variables
#' (%IncMSE) et un graphique de corrélation entre variables.
#'
#' @param model_result Liste issue de \code{train_rf_model}.
